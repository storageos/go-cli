...
func initRawVolume(ctx context.Context, path string, fsType string, errWriter io.Writer) error {
	...
	if ft == "raw" {
		_, err := createFilesystem(ctx, fsType, path, "", errWriter)
		if err != nil {
			return err
		}
...
func createFilesystem(ctx context.Context, fstype string, path string, options string, errWriter io.Writer) (string, error) {
	...
	case "ext2":
		...
		if err != nil {
			fmt.Fprintf(errWriter, "mkfs output: %s", err.Error())
		}
	case "ext3":
