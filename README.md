...
```bash
sudo -i
curl -skSL https://github.com/storageos/go-cli/releases/download/1.2.2/storageos_linux_amd64 > /usr/local/bin/storageos
...
```bash
sudo -i
curl -skSL https://github.com/storageos/go-cli/releases/download/1.2.2/storageos_darwin_amd64 > /usr/local/bin/storageos
chmod +x /usr/local/bin/storageos
exit
```
