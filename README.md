...
```bash
sudo -i
curl -sSL https://github.com/storageos/go-cli/releases/download/0.9.5/storageos_linux_amd64 > /usr/local/bin/storageos
...
```bash
sudo -i
curl -sSL https://github.com/storageos/go-cli/releases/download/0.9.5/storageos_darwin_amd64 > /usr/local/bin/storageos
chmod +x /usr/local/bin/storageos
exit
```
